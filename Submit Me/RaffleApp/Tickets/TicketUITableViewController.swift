...
class TicketUITableViewController: UITableViewController, ShareTicketProtocol {
    ...
    func shareTicketInfo(_ customerName: String, _ customerTicketNum: String, _ customerSoldTime: String) {
            let shareViewController = UIActivityViewController(
                activityItems: ["\(customerName)'s ticket number \(customerTicketNum) sold at \(customerSoldTime) for \(raffle!.raffle_name) which is drawn at \(raffle!.draw_date). The prize is $ \(raffle!.prize)"], applicationActivities: [])
